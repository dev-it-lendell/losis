            ...
            if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)) {
                ...
                if (is_uploaded_file($_FILES['file']['tmp_name'])) {
                    ...
                    while (($line = fgetcsv($csvFile)) !== FALSE) {
                        ...
                        $prevQuery = "SELECT fname, mname, lname,    endo_date FROM tbl_endo 
                        WHERE fname = '".$line[1]."' AND mname = '".$line[2]."' AND lname = '".$line[3]."' AND suffix = '".$line[4]."'";
                        $prevResult = $conn->query($prevQuery); 

                        if ($prevResult->num_rows > 0) {
                        // Initialize $dupDetails as an array
                        $dupDetails = array();

                        // Collecting details of the duplicate entries
                        while ($dupRow = $prevResult->fetch_assoc()) {
                        $dupDetails[] = 'Name: ' . $dupRow['fname'] . ' ' . $dupRow['mname'] . ' ' . $dupRow['lname'] . ' ' . $dupRow['suffix'] . 
                                '<br>Endorsement Date: ' . date('Y-m-d', strtotime($dupRow['endo_date']));
                        }

                        // Convert array to string for URL encoding
                        $dupDetailsString = implode('<hr>', $dupDetails); // Separate entries with a horizontal line
                        $qstring = '?status=dup_entry&dupDetails=' . urlencode($dupDetailsString);

                        // Redirect to display the SweetAlert with duplicate details
