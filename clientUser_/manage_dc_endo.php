            ...
            case 'err':
                ?>
                    <script type="text/javascript">
                        ...
                        toastr.options.closeButton = true;
                        toastr.options.positionClass = 'toast-bottom-right';
                        toastr['danger']('Cant create endorsements.');
                    </script>
                <?php
            break;
                                    ...
                                    <div class="card-body">
                                        <div class="media mleft">
                                            ...
                                            <div class="media-body">
                                                ...
                                                <p style="font-size: 25px; text-align: right; margin-top: -55px; font-weight: bold;" id="dccurrenttotalendo"></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            	...
                            	<div class="col-md-12">
                            		...
                            		<div class="table-responsive">
                            			<table class="table table-hover js-basic-example dataTable table-custom table-striped m-b-0 c_list">
                                            ...
                                            <tbody>
                                            	<?php
                                            		$sql = "SELECT a.id, a.endo_id, a.endo_desc, a.endo_code, a.endo_date, a.endo_status, a.is_for_review, a.is_done, a.endorsed_to, a.turn_around_date, a.endo_services, CONCAT(a.fname, ' ', a.mname, ' ', a.lname, ' ', a.suffix) AS applicantname, a.hired_date, a.address, a.birthdate, a.site_id, a.client_id, a.endo_requestor, a.account, a.initiation_date, a.bi_id, a.bi_batch, a.importance, a.closure_date, a.package_desc, a.change_package, b.user_id, CONCAT(b.fname, ' ', b.mname, ' ', b.lname, ' ', b.suffix) AS clientname, b.company_name, b.user_position, b.site_id, c.user_id, CONCAT(c.user_id) AS supervisorid, CONCAT(c.fname,  ' ', c.mname, ' ', c.lname, ' ', c.suffix) AS supervisorname, d.client_id, d.client_name, d.acronym_, d.site_ FROM tbl_endo_criminal AS a LEFT JOIN tbl_client AS b ON a.client_id = b.user_id LEFT JOIN tbl_supervisor AS c ON a.endorsed_to = c.user_id LEFT JOIN client_list AS d ON a.site_id = d.client_id WHERE MONTH(a.endo_date) = MONTH(NOW()) AND YEAR(a.endo_date) = YEAR(NOW()) AND b.company_name  = '$clnt_company' ORDER BY a.importance ASC";
                                                            ...
                                                            ?>
                                                            	<tr>
                                                                    ...
                                                                    <td style="text-align: center;">
                                                                    	<?php
                                                                    		if ($endo_status == '0' || $endo_status == '1' || $endo_status == '2' || $endo_status == '3' && $is_for_review == '1') {
                                                                    			?>
                                                                    				<?php
                                                                    					if ($change_package == '1') {
                                                                    						?>
																							<button type="button" class="btn btn-sm btn-outline-dark" data-toggle="tooltip" data-placement="top" title="View Endorsement" onclick="showviewendodc('<?php echo $row['applicantname']; ?>', '<?php echo $newDateEndo; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['endo_desc']; ?>', '<?php echo $newTurnAroundDate; ?>', '<?php echo $row['clientname']; ?>', '<?php echo $row['company_name']. ' - ' .$row['site_']; ?>', '<?php echo $row['package_desc']; ?>'); saveviewendodc();"><i class="fa fa-file-text-o"></i></button>
                                                                    						<?php
                                                                    					} else {
                                                                    						?>
                                                                    						<button type="button" class="btn btn-sm btn-outline-dark" data-toggle="tooltip" data-placement="top" title="View Endorsement" onclick="showviewendodc('<?php echo $row['applicantname']; ?>', '<?php echo $newDateEndo; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['endo_desc']; ?>', '<?php echo $newTurnAroundDate; ?>', '<?php echo $row['clientname']; ?>', '<?php echo $row['company_name']. ' - ' .$row['site_']; ?>', '<?php echo $row['package_desc']; ?>'); saveviewendodc();"><i class="fa fa-file-text-o"></i></button>
                                                                    						<button type="button" class="btn btn-sm btn-outline-info" data-toggle="tooltip" data-placement="top" title="Upgrade Package" onclick="saveupgradepkgdc('<?php echo $row['package_desc']; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['supervisorid']; ?>'); saveupgradepkgbi();"><i class="fa  fa-pencil-square-o"></i></button>
                                                                    						<?php
                                                                    					}
                                                                    				?>
                                                                    			<?php
                                                                    			...
                                                                    			?>
                                                                    				<?php
                                                                    					if ($change_package == '1') {
                                                                    						?>
                                                                    						<button type="button" class="btn btn-sm btn-outline-dark" data-toggle="tooltip" data-placement="top" title="View Endorsement" onclick="showviewendodc('<?php echo $row['applicantname']; ?>', '<?php echo $newDateEndo; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['endo_desc']; ?>', '<?php echo $newTurnAroundDate; ?>', '<?php echo $row['clientname']; ?>', '<?php echo $row['company_name']. ' - ' .$row['site_']; ?>', '<?php echo $row['package_desc']; ?>'); saveviewendodc();"><i class="fa fa-file-text-o"></i></button>
                                                                                            <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="top" title="Return Endorsement" onclick="showpocreturnendodc('<?php echo $row['applicantname']; ?>', '<?php echo $row['supervisorname']; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['supervisorid']; ?>'); savereturnendodc();"><i class="fa fa-undo"></i></button>
                                                                                    		<a onclick="savedownloadrepdc();" href="download_report_dc.php?id=<?=$row['endo_id'];?>" class="btn btn-sm btn-outline-success" data-toggle="tooltip" data-placement="top" title="Download Report"><i class="fa fa-download"></i></a>
                                                                    						<?php
                                                                    					} else {
                                                                    						?>
                                                                    						<button type="button" class="btn btn-sm btn-outline-dark" data-toggle="tooltip" data-placement="top" title="View Endorsement" onclick="showviewendodc('<?php echo $row['applicantname']; ?>', '<?php echo $newDateEndo; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['endo_desc']; ?>', '<?php echo $newTurnAroundDate; ?>', '<?php echo $row['clientname']; ?>', '<?php echo $row['company_name']. ' - ' .$row['site_']; ?>', '<?php echo $row['package_desc']; ?>'); saveviewendodc();"><i class="fa fa-file-text-o"></i></button>
                                                                    						<button type="button" class="btn btn-sm btn-outline-info" data-toggle="tooltip" data-placement="top" title="Upgrade Package" onclick="showupgradepkgdc('<?php echo $row['package_desc']; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['supervisorid']; ?>'); saveupgradepkgdc();"><i class="fa  fa-pencil-square-o"></i></button>
                                                                    						<button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="top" title="Return Endorsement" onclick="showpocreturnendodc('<?php echo $row['applicantname']; ?>', '<?php echo $row['supervisorname']; ?>', '<?php echo $row['endo_code']; ?>', '<?php echo $row['supervisorid']; ?>'); savereturnendodc();"><i class="fa fa-undo"></i></button>
                                                                                    		<a onclick="savedownloadrepdc();" href="download_report_dc.php?id=<?=$row['endo_id'];?>" class="btn btn-sm btn-outline-success" data-toggle="tooltip" data-placement="top" title="Download Report"><i class="fa fa-download"></i></a>
                                                                    						<?php
                                                                    					}
