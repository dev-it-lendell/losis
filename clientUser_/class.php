<?php
	...
	switch ($_POST['form']) {
		...
		case 'showonprocessendo':
            $sqlbi = "SELECT COUNT(a.id) AS onprocess, a.client_id, b.user_id, b.company_name FROM tbl_endo AS a LEFT JOIN tbl_client AS b ON a.client_id = b.user_id WHERE YEAR(a.endo_date) = YEAR(NOW()) AND a.endo_status = '1' OR a.endo_status = '2' AND b.company_name = '$clnt_company' AND MONTH(a.endo_date) = MONTH(NOW())";
            $res = $conn->query($sqlbi);
            $row = mysqli_fetch_array($res);
            $sqldc = "SELECT COUNT(a.id) AS onprocess, a.client_id, b.user_id, b.company_name FROM tbl_endo_criminal AS a LEFT JOIN tbl_client AS b ON a.client_id = b.user_id WHERE YEAR(a.endo_date) = YEAR(NOW()) AND a.endo_status = '1' OR a.endo_status = '2' AND b.company_name = '$clnt_company' AND MONTH(a.endo_date) = MONTH(NOW())";
            $res1 = $conn->query($sqldc);
            $row1 = mysqli_fetch_array($res1);
		...
		case 'showprevonprocessendo':
            $sqlbi = "SELECT COUNT(a.id) AS onprocprevendo, a.client_id, b.user_id, b.company_name FROM tbl_endo AS a LEFT JOIN tbl_client AS b ON a.client_id = b.user_id WHERE a.endo_status = '1' OR a.endo_status = '2' AND YEAR(a.endo_date) = YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AND MONTH(a.endo_date)  = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) AND b.company_name = '$clnt_company'";
            $res = $conn->query($sqlbi);
            $row = mysqli_fetch_array($res);
        ...
        case 'showcurrentotalappl':
            $sql = "SELECT COUNT(id) AS currentappl FROM application_list WHERE MONTH(application_datetime) = MONTH(NOW()) AND YEAR(application_datetime) = YEAR(NOW()) AND client_name = '". $clnt_company ."'";
            $res = $conn->query($sql);
            $row = mysqli_fetch_array($res);
        ...
        case 'showapplstatus':
            $sql = "SELECT COUNT(id) AS pending FROM application_list WHERE MONTH(application_datetime) = MONTH(NOW()) AND YEAR(application_datetime) = YEAR(NOW()) AND application_status = '0' AND client_name = '". $clnt_company ."'";
            $res = mysqli_query($conn, $sql);
            $sql1 = "SELECT COUNT(id) AS approved FROM application_list WHERE MONTH(application_datetime) = MONTH(NOW()) AND YEAR(application_datetime) = YEAR(NOW()) AND application_status = '1' AND client_name = '". $clnt_company ."'";
            $res1 = mysqli_query($conn, $sql1);
            $row1 = mysqli_fetch_array($res1);
            $sql2 = "SELECT COUNT(id) AS disapproved FROM application_list WHERE MONTH(application_datetime) = MONTH(NOW()) AND YEAR(application_datetime) = YEAR(NOW()) AND application_status = '2' AND client_name = '". $clnt_company ."'";
            $res2 = mysqli_query($conn, $sql2);
            $row2 = mysqli_fetch_array($res2);
            while ($row = mysqli_fetch_array($res)) {
                $totalpending = $row['pending'];
