<?php
    ...
    if (isset($_POST['exportadmBI'])) {
		$admbi_start_date = $_POST['admbi_start_date'] ?? null;
		$admbi_end_date = $_POST['admbi_end_date'] ?? null;

		$sql = "SELECT a.id, a.endo_id, a.endo_desc, a.endo_code, CONCAT(a.fname, ' ', a.mname, ' ', a.lname, ' ', a.suffix) AS applicantname, a.birthdate, a.endo_date, a.endo_status, a.is_for_review, a.is_done, a.endorsed_to, a.turn_around_date, a.endo_services, a.package_desc, a.client_id, a.site_id, a.endo_requestor, a.change_package, a.importance, a.bi_id, a.closure_date, a.account, a.initiation_date, b.endo_code, b.assigned_supervisor, b.assigned_support, b.assigned_specialist, b.assigned_analyst, b.assigned_client, b.percentage_, c.client_id, c.client_name, c.acronym_, c.site_, c.team_, d.team_no, d.team_name, e.user_id, CONCAT(e.fname, ' ', e.mname,  ' ', e.lname, ' ', e.suffix) AS clientname, f.user_id, CONCAT(f.fname, ' ', f.mname,  ' ', f.lname, ' ', f.suffix) AS supervisorname, g.user_id, CONCAT(g.fname, ' ', g.mname,  ' ', g.lname, ' ', g.suffix) AS supportname, h.user_id, CONCAT(h.fname, ' ', h.mname,  ' ', h.lname, ' ', h.suffix) AS operationstelename FROM tbl_endo AS a LEFT JOIN tbl_endorsement_bi_process AS b ON a.endo_code = b.endo_code LEFT JOIN client_list AS c ON a.site_id = c.client_id LEFT JOIN team_list AS d ON c.team_ = d.team_no LEFT JOIN tbl_client AS e ON b.assigned_client = e.user_id LEFT JOIN tbl_supervisor AS f ON b.assigned_supervisor = f.user_id LEFT JOIN tbl_support AS g ON b.assigned_support = g.user_id LEFT JOIN tbl_operations AS h ON b.assigned_specialist = h.user_id WHERE a.endo_date BETWEEN '$admbi_start_date' AND '$admbi_end_date' ORDER BY id DESC";
 		...
 		if (mysqli_num_rows($result) > 0) {
			  $output .= '
			   <table class="table" bordered="1">  
	                <tr>  
	                    ...
	                    <th>ASSIGNED TELE SPECIALIST</th>
			  ...
			  while ($row = mysqli_fetch_array($result)) {
		            ...
		            $operationstelename = $row['operationstelename'] ?? null;
					...
					$sql1 = "SELECT a.id, a.endo_id, a.endo_desc, a.endo_code, CONCAT(a.fname, ' ', a.mname, ' ', a.lname, ' ', a.suffix) AS applicantname, a.birthdate, a.endo_date, a.endo_status, a.is_for_review, a.is_done, a.endorsed_to, a.turn_around_date, a.endo_services, a.package_desc, a.client_id, a.site_id, a.endo_requestor, a.change_package, a.importance, a.bi_id, a.closure_date, a.account, a.initiation_date, b.endo_code, b.assigned_supervisor, b.assigned_support, b.assigned_specialist, b.assigned_analyst, b.assigned_client, b.percentage_, c.client_id, c.client_name, c.acronym_, c.site_, c.team_, d.team_no, d.team_name, e.user_id, CONCAT(e.fname, ' ', e.mname,  ' ', e.lname, ' ', e.suffix) AS operationsdaname FROM tbl_endo AS a LEFT JOIN tbl_endorsement_bi_process AS b ON a.endo_code = b.endo_code LEFT JOIN client_list AS c ON a.site_id = c.client_id LEFT JOIN team_list AS d ON c.team_ = d.team_no LEFT JOIN tbl_operations AS e ON b.assigned_analyst = e.user_id WHERE a.endo_date BETWEEN '$admbi_start_date' AND '$admbi_end_date' ORDER BY id DESC";
					$result1 = $conn->query($sql1);
					$row1 = mysqli_fetch_array($result1);
		            $operationsdaname = $row1['operationsdaname'] ?? null;
			...
			header('Content-Disposition: attachment; filename=adminbiendorsements.xls');
			echo $output;
			$sql = "INSERT INTO tbl_admin_history_logs SET
						user_id = '". $_SESSION['user_id'] ."',
						x_module = 'Dashboard',
						x_module_action = 'Download Exported Data - (BI)'";
			$res = $conn->query($sql);                                
			if ($res) {
