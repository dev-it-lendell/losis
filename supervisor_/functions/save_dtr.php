<?php
    ...
    if (isset($_POST['importDTR'])) {
			...
			if (is_uploaded_file($_FILES['file']['tmp_name'])) {
				...
				while (($line = fgetcsv($csvFile)) !== FALSE) {
                    ...
                    for ($count = 0; $count < count($activity); $count++) { 
						$conn->query("INSERT INTO dtr_supervisor (dtr_id, supervisor_id, assigned_team, date_created, activity_, quantity_, status_, datetime_added) VALUES('".$dtrCode."' ,'". $_SESSION['user_id'] ."', '".$team_."', '".$newinputdate."', '".$activity."', '".$quantity."', '".$activitystatus."', NOW())");
        ...
        header("Location: ../dtr_management_poc.php".$qstring);  
    }
?>