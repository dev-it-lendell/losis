...
require('../functions/protection_pdf.php');
...
while ($row = mysqli_fetch_array($query)) {
	...
	$pdf->SetProtection(['print'], '123', '123');
	...
	$pdf->Cell(10, 4, '', 0, 1); //end of line$pdf->Cell(10 ,4,'',0,1);//end of line
	// IDENTITY CHECK (SSS) //
	$pdf->SetFont('Century Gothic', 'B', 8);
	$pdf->setFillColor(230, 230, 230);
	if ($identity_sss == '1') {
	...
	// $pdf = new PDF_Protection();
	// $pdf->SetProtection(array('print'), 'CONCENTRIX', 'CONCENTRIX');
	// $pdf->SetProtection(array('print'), 'CONCENTRIX', 'CONCENTRIX');
	// $pdf->SetProtection(['print'], '1234', '1234');
	// $pdf->SetProtection(['print'], '', 'CONCENTRIX');
	// $pdf->AddPage();
	// $pdf->SetFont('Arial', '', 14);
	// $pdf->Cell(0, 10, 'This PDF is password protected!');
	// $pdf->Output('F', 'protected.pdf'); // Save to file
	// $pdf->Output('I');
	$pdf->Output();
}